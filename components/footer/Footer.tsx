import Image from "next/image";
import Link from "next/link";
import { Mail, MapPin, Phone } from "lucide-react";
import { FaFacebookF, FaLinkedinIn, FaInstagram } from "react-icons/fa";
import { Separator } from "../ui/separator";

export function Footer() {
  return (
    <footer className="bg-madmon-main text-background py-12 relative z-40">
      <div className="container mx-auto px-4">
        <div className="flex gap-11 items-stretch justify-center mx-auto">
          {/* Logo */}
          <div>
            <Image
              src="/assets/images/logo-white.png"
              alt="Madmon Logo"
              width={150}
              height={60}
              className="w-44"
            />
          </div>
          <Separator orientation="vertical" className="h-44" />
          {/* Contact Info */}
          <div className="space-y-5">
            <h3 className="text-lg font-semibold mb-1 text-madmon-info">
              Contact Us
            </h3>
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                <MapPin className="h-5 w-5" />
                <span>42 Fareek Awai Ali Amer</span>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="h-5 w-5" />
                <span>01234568910</span>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="h-5 w-5" />
                <span>hr@Madmon.ai</span>
              </div>
            </div>
          </div>

          <Separator orientation="vertical" className="h-44" />

          {/* Links and Social */}
          {/* Useful Links */}
          <div className="space-y-2 flex flex-col justify-between">
            <h3 className="text-lg font-semibold mb-1 text-madmon-info">
              Useful Links
            </h3>
            <ul className="space-y-3">
              <li>
                <Link href="/about" className="hover:underline">
                  About Us
                </Link>
              </li>
              <li>
                <Link href="/privacy" className="hover:underline">
                  Privacy Policy
                </Link>
              </li>
              <li>
                <Link href="/support" className="hover:underline">
                  Support
                </Link>
              </li>
              <li>
                <Link href="/terms" className="hover:underline">
                  Terms & Conditions
                </Link>
              </li>
            </ul>
          </div>

          <Separator orientation="vertical" className="h-44" />

          {/* Social Icons */}
          <div className="flex justify-center items-center h-auto gap-4 md:justify-end">
            <Link
              href="#"
              className="border p-2 rounded-md hover:bg-white/20 transition-colors"
            >
              <FaLinkedinIn className="h-5 w-5" />
            </Link>
            <Link
              href="#"
              className="border p-2 rounded-md hover:bg-white/20 transition-colors"
            >
              <FaInstagram className="h-5 w-5" />
            </Link>
            <Link
              href="#"
              className="border p-2 rounded-md hover:bg-white/20 transition-colors"
            >
              <FaFacebookF className="h-5 w-5" />
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
